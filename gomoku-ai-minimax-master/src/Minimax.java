...
public class Minimax {
	...
	public static double evaluateBoardForBlack(Board board, boolean blacksTurn) {
		evaluationCount++;
	...
	public int[] calculateNextMove(int depth) {
		...
		totalWhiteTime += (System.currentTimeMillis() - startTime);
		System.out.println("MiniMax Cases calculated: " + evaluationCount + " Calculation time: " + (System.currentTimeMillis() - startTime) + " ms" + " Total White time: "+ totalWhiteTime + " ms");
	...
	public int[] calculateNextMoveForBlack(int depth) {
		board.thinkingStarted();
		int[] move = new int[2];
		long startTime = System.currentTimeMillis();
		// Check if any available move can finish the game
		...
		if(bestMove != null ) {
			...
			// If there is no such move, search the minimax tree with suggested depth.
			bestMove = minimaxSearchABForBlack(depth, board, true, -1.0, getWinScore());
			if(bestMove[1] == null) {
				move = null;
			} else {
				move[0] = (Integer)(bestMove[1]);
	...
	public int[] calculateNextMoveForBlackHeurstic(int depth) {
		board.thinkingStarted();
		int[] move = new int[2];
		long startTime = System.currentTimeMillis();
		// Check if any available move can finish the game
		...
		if(bestMove != null ) {
			...
			// If there is no such move, search the minimax tree with suggested depth.
			double blackScore = getScore(board, true, true);
			double whiteScore = getScore(board, false, true);
			if(whiteScore >= 10 * blackScore) {
				depth = 2;
			}
			if(whiteScore > 2 * blackScore && whiteScore < 10 * blackScore) {
				depth = 3;
			}
			if(whiteScore <= 2 * blackScore) {
				depth = 4;
			}
			bestMove = minimaxSearchABForBlack(depth, board, true, -1.0, getWinScore());
			if(bestMove[1] == null) {
	...
	private static Object[] minimaxSearchABForBlack(int depth, Board board, boolean max, double alpha, double beta) {
		if(depth == 0) {
			
			Object[] x = {evaluateBoardForBlack(board, !max), null, null};
			return x;
		}
